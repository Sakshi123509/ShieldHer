...
function BlueBG() {
  ...
  useEffect(() => {
    ...
    const render = (now) => {
      ...
      const bg = ctx.createRadialGradient(W * .5, H * .35, 0, W * .5, H * .5, Math.max(W, H) * .8);
      bg.addColorStop(0, "#060f22");
      bg.addColorStop(.5, "#040c1a");
      bg.addColorStop(1, "#020810");
      ctx.fillStyle = bg;
      ctx.fillRect(0, 0, W, H);

      // Blue ambient glow blobs
      [[.18, .22, .28], [.82, .72, .22], [.5, .9, .18]].forEach(([rx, ry, s]) => {
        const g = ctx.createRadialGradient(W * rx, H * ry, 0, W * rx, H * ry, Math.min(W, H) * s);
        g.addColorStop(0, `rgba(0,80,160,${.07 + Math.sin(t * .4 + rx * 8) * .025})`);
      ...
      ptsRef.current.forEach(p => {
        ...
        const a = p.alpha * (.4 + Math.sin(p.pulse) * .4);
        ctx.save();
        ctx.globalAlpha = a * .8;
      ...
      for (let i = 0; i < 3; i++) {
        const ph = ((t * .2 + i / 3) % 1);
        ctx.save();
        ctx.globalAlpha = (1 - ph) * .04;
        ctx.strokeStyle = "#00cfff";
        ctx.lineWidth = 1;
        ctx.beginPath();
        ctx.arc(W * .5, H * .5, ph * Math.min(W, H) * .55, 0, Math.PI * 2);
        ctx.stroke();
        ctx.restore();
      }
    ...
    <div style={{
      ...
      <div style={{
        ...
        <span style={{
          ...
          letterSpacing: "2.5px", color: "rgba(232, 235, 236, 0.92)", textTransform: "uppercase",
        }}>{title}</span>
      </div>
        ...
        <div key={i} style={{ display: "flex", alignItems: "flex-start" }}>
          <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: "6px" }}>
            ...
            <div style={{
              ...
              border: `1.5px solid ${i <= current ? "#00cfff" : "rgba(0,180,255,0.2)"}`,
              background: i < current ? "#00cfff" : i === current ? "rgba(0,207,255,0.12)" : "transparent",
              display: "flex", alignItems: "center", justifyContent: "center",
              boxShadow: i <= current ? "0 0 14px rgba(0,207,255,0.35)" : "none",
              transition: "all .3s",
            }}>
              {i < current
                ...
                : <span style={{ fontFamily: "'Courier New', monospace", fontSize: "10px", color: i === current ? "#00cfff" : "rgba(0,180,255,0.3)", fontWeight: 700 }}>{i + 1}</span>
              }
            </div>
            <span style={{
              fontSize: "9px", letterSpacing: "1px", textTransform: "uppercase",
              color: i <= current ? "rgba(0,207,255,0.7)" : "rgba(0,180,255,0.25)",
          ...
          {i < labels.length - 1 && (
            <div style={{
              width: "80px", height: "1px", marginTop: "14px",
              background: i < current ? "#00cfff" : "rgba(0,180,255,0.15)",
              boxShadow: i < current ? "0 0 6px rgba(0,207,255,0.4)" : "none",
              transition: "all .3s",
            }} />
          )}
        </div>
    ...
    <div style={{ display: "flex", gap: "12px", marginTop: "24px" }}>
      {onBack && (
        <button onClick={onBack} style={{
          ...
          border: "1px solid rgba(0,180,255,0.3)", borderRadius: "6px",
          color: "rgba(0,207,255,0.55)", fontSize: "12px", letterSpacing: "2px",
          fontFamily: "'Courier New', monospace", cursor: "pointer", transition: "all .2s",
          flexShrink: 0,
        }}
      ...
      <button onClick={onNext} disabled={nextDisabled} style={{
        ...
        background: nextDisabled
          ? "rgba(0,180,255,0.06)"
          : "linear-gradient(90deg,rgba(0,100,200,0.5),rgba(0,207,255,0.4))",
        border: `1px solid ${nextDisabled ? "rgba(0,180,255,0.15)" : "rgba(0,207,255,0.55)"}`,
        borderRadius: "6px",
        color: nextDisabled ? "rgba(128, 184, 208, 0.59)" : "#7de8ff",
        fontSize: "12px", letterSpacing: "2.5px",
        fontFamily: "'Courier New', monospace", fontWeight: 700,
        cursor: nextDisabled ? "not-allowed" : "pointer",
        boxShadow: nextDisabled ? "none" : "0 0 24px rgba(0,207,255,0.18)",
        transition: "all .25s",
      }}>
        {nextLabel || "NEXT →"}
      </button>
...
function ContactCard({ idx, contact, onChange, onRemove, canRemove }) {
  ...
  return (
    <div style={{
      background: "rgba(4,18,48,0.82)",
      border: `1px solid ${open ? "rgba(0,180,255,0.3)" : "rgba(0,180,255,0.12)"}`,
      ...
      <div onClick={() => setOpen(o => !o)} style={{
        ...
        borderBottom: open ? "1px solid rgba(0,180,255,0.1)" : "none",
        background: open ? "rgba(0,207,255,0.03)" : "transparent",
        userSelect: "none",
      }}>
        <div style={{ display: "flex", alignItems: "center", gap: "12px" }}>
          ...
          <div>
            <div style={{ fontFamily: "'Courier New', monospace", fontSize: "10px", letterSpacing: "2px", color: accent }}>
              CONTACT {idx + 1}{idx < 3 && <span style={{ color: "rgba(0,207,255,0.35)", marginLeft: 6, fontSize: "9px" }}>REQUIRED</span>}
            </div>
            {contact.name && (
              <div style={{ fontSize: "13px", color: "rgba(168,240,255,0.65)", marginTop: "2px" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
          {canRemove && (
            <button onClick={e => { e.stopPropagation(); onRemove(); }} style={{
              background: "rgba(0,100,200,0.1)", border: "1px solid rgba(0,150,220,0.3)",
              borderRadius: "5px", color: "rgba(0,207,255,0.6)", fontSize: "11px",
        ...
        <div style={{ padding: "18px", display: "flex", flexDirection: "column", gap: "14px" }}>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "12px" }}>
            <Field label="Full Name" required>
              <input type="text" placeholder="Jane Doe" value={contact.name}
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "12px" }}>
            ...
            <Field label="Email">
              <input type="email" placeholder="jane@email.com" value={contact.email}
          ...
          <div style={{ display: "flex", alignItems: "center", gap: "12px" }}>
            ...
            {[1, 2, 3].map(p => (
              <button key={p} onClick={() => onChange({ ...contact, priority: p })} style={{
                width: "28px", height: "22px", borderRadius: "4px",
                border: `1px solid ${(contact.priority || 1) >= p ? "rgba(0,207,255,0.6)" : "rgba(0,180,255,0.18)"}`,
                background: (contact.priority || 1) >= p ? "rgba(0,207,255,0.12)" : "transparent",
                color: (contact.priority || 1) >= p ? "#00cfff" : "rgba(0,180,255,0.25)",
                fontSize: "11px", cursor: "pointer",
                fontFamily: "'Courier New', monospace", transition: "all .15s",
              }}>{p}</button>
            ))}
            <span style={{ fontSize: "11px", color: "rgba(0,180,255,0.4)" }}>
    ...
    <>
      ...
      <div style={{ position: "relative", minHeight: "100vh", background: "#060d1a" }}>
        ...
        <nav style={{
          ...
          <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
            <div style={{
              width: "32px", height: "32px", border: "1.5px solid rgba(0,207,255,0.6)",
              borderRadius: "7px", display: "flex", alignItems: "center", justifyContent: "center",
              background: "rgba(0,207,255,0.07)", boxShadow: "0 0 12px rgba(0,207,255,0.2)",
            }}>
              <svg width="16" height="16" viewBox="0 0 20 20" fill="none">
                <path d="M10 2L17 5L17 10C17 14.5 13.5 17.5 10 19C6.5 17.5 3 14.5 3 10L3 5Z"
                  fill="rgba(0,207,255,0.2)" stroke="#00cfff" strokeWidth="1.5"/>
          ...
          <span style={{ fontFamily: "'Courier New', monospace", fontSize: "18px", letterSpacing: "3px", color: "rgba(33, 175, 207, 0.92)" }}>
            PROFILE SETUP
          </span>
        ...
        <main style={{ position: "relative", zIndex: 1, maxWidth: "720px", margin: "0 auto", padding: "96px 20px 60px" }}>
              ...
              <Card icon="👤" title="Basic Information">
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "14px" }}>
                  <Field label="First Name" required>
                    <input type="text" placeholder="Priya" value={personal.firstName}
                      onChange={e => setPersonal(p => ({ ...p, firstName: e.target.value }))} />
                  </Field>
                  <Field label="Last Name">
                    <input type="text" placeholder="Sharma" value={personal.lastName}
                      onChange={e => setPersonal(p => ({ ...p, lastName: e.target.value }))} />
                  </Field>
                  <Field label="Age" required hint="1 – 100">
                    <input type="number" placeholder="25" min="1" max="100" value={personal.age}
                  ...
                  <Field label="Your Phone" required>
                    <input type="tel" placeholder="+91 99999 00000" value={personal.phone}
                ...
                <Field label="Medical Notes" hint="Optional">
                  <textarea rows={3} placeholder="Allergies, conditions, or important health info..."
                    value={personal.medicalNotes}
                    onChange={e => setPersonal(p => ({ ...p, medicalNotes: e.target.value }))}
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: "14px", animation: "fadeUp .4s ease both" }}>
              ...
              <div style={{
                ...
                <p style={{ fontSize: "13px", color: "rgba(168,240,255,0.55)", lineHeight: 1.65, fontFamily: "'Courier New', monospace" }}>
                  Add <strong style={{ color: "#7de8ff" }}>minimum 3</strong>, up to <strong style={{ color: "#7de8ff" }}>5 contacts</strong>.
                  They get an instant call + SMS with your live GPS when ShieldHer activates.
                </p>
              </div>

              {contacts.map((c, i) => (
                <ContactCard key={i} idx={i} contact={c}
              ...
              {contacts.length < 5 && (
                <button onClick={addContact} style={{
                  ...
                  color: "rgba(0,207,255,0.5)", fontSize: "11px",
                  ...
                  ADD CONTACT {contacts.length + 1} OF 5
                </button>
              )}
              ...
              <div style={{ textAlign: "center" }}>
                <span style={{
                  ...
                  color: contacts.length >= 3 ? "rgba(0,207,255,0.65)" : "rgba(0,180,255,0.35)",
                  fontFamily: "'Courier New', monospace",
                }}>
          ...
          {step === 2 && (
            <div style={{ display: "flex", flexDirection: "column", gap: "18px", animation: "fadeUp .4s ease both" }}>
              <Card icon="🏠" title="Home Address">
                <Field label="Address Line 1" required>
                  <input type="text" placeholder="Flat / House No, Building, Street"
              ...
              {/* Safe zone radius */}
              <Card icon="📍" title="Safe Zone Radius">
                ...
                <Field label="Radius" hint={`${Number(address.radius).toLocaleString()} m`}>
                  <input type="range" min="100" max="5000" step="100"
                    ...
                    onChange={e => setAddress(a => ({ ...a, radius: e.target.value }))}
                    style={{
                      accentColor: "#00cfff",
                      background: `linear-gradient(90deg,#00cfff ${pct}%,rgba(0,180,255,0.15) ${pct}%)`,
                    }} />
                </Field>
                <div style={{ display: "flex", justifyContent: "space-between" }}>
                  {["100 m","1 km","2.5 km","5 km"].map(l => (
                    <span key={l} style={{ fontSize: "10px", color: "rgba(0,180,255,0.3)", fontFamily: "'Courier New', monospace" }}>{l}</span>
                  ))}
                </div>
              </Card>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: "14px" }}>
                {saved && (
                  <div style={{
                    ...
                    <svg width="15" height="15" viewBox="0 0 15 15" fill="none">
                      <polyline points="2,7.5 5.5,11 13,3.5" stroke="#00c864" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" />
                    </svg>
                    <span style={{ color: "#00c864", fontFamily: "'Courier New', monospace", fontSize: "10px", letterSpacing: "2px" }}>
                ...
                <div style={{ display: "flex", gap: "12px" }}>
                  <button onClick={() => setStep(1)} style={{
                    ...
                    background: "transparent",
                    border: "1px solid rgba(0,180,255,0.3)", borderRadius: "6px",
                    color: "rgba(0,207,255,0.55)", fontSize: "11px",
                    letterSpacing: "2px", fontFamily: "'Courier New', monospace",
                  ...
                  <button onClick={save} disabled={!ok2} style={{
                    flex: 1, padding: "15px",
                    background: ok2
                      ? "linear-gradient(90deg,rgba(0,100,200,0.55),rgba(0,207,255,0.45))"
                      : "rgba(0,180,255,0.06)",
                    border: `1px solid ${ok2 ? "rgba(0,207,255,0.6)" : "rgba(0,180,255,0.15)"}`,
                    borderRadius: "6px",
                    color: ok2 ? "#7de8ff" : "rgba(0,180,255,0.3)",
                    fontSize: "12px", letterSpacing: "2.5px",
                    fontFamily: "'Courier New', monospace", fontWeight: 700,
                    cursor: ok2 ? "pointer" : "not-allowed",
                    boxShadow: ok2 ? "0 0 28px rgba(0,207,255,0.2)" : "none",
                    animation: ok2 ? "pulseRing 2.5s ease-in-out infinite" : "none",
                    transition: "all .3s",
                  }}>
